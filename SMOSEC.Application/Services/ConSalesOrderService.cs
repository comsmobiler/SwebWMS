...
namespace SMOWMS.Application.Services
{
    ...
    public class ConSalesOrderService : IConSalesOrderService
    {
        ...
        /// </summary>
        /// <returns></returns>
        public List<Object> GetCustomerAnalyzeEchart(DateTime startDate, DateTime endDate)
        {
            List<ConSalesOrderRow> orderRows = _ConSalesOrderRowReposity.GetAll()
                .Where(x => (x.STATUS == 1 || x.STATUS == 2) && x.CREATEDATE >= startDate
                && x.CREATEDATE <= endDate).ToList();
            ...
            foreach (ConSalesOrderRowInputDto row in Data)
            {
                if (customers.Contains(row.CUSTOMERNAME) == false)
                    customers.Add(row.CUSTOMERNAME);
                BarChartDto dto = new BarChartDto();
                dto.Series = row.CUSTOMERNAME;
                dto.XValue = row.NAME;
                dto.YValue = row.QUANTOUT;
            ...
            {
                resultList.Add(customers);
                resultList.Add(barChartDtos);
                return resultList;
            }
            else
                return null;
