...
namespace SMOWMS.Application.Services
{
    ...
    public class AssAnalysisService : IAssAnalysisService
    {
        ...
        {
            var asset = _assetsRepository.GetAll().
                Where(a => a.ISINWAREHOUSE == 1 && a.EXPIRYDATE >= inputDto.StarTime && a.EXPIRYDATE <= inputDto.EndTime).
                OrderBy(a => a.EXPIRYDATE).OrderBy(a => a.EXPIRYDATE).ToList();
            var all = asset.Select(a => new AssImminentExpiryDto() { ASSID = a.ASSID, SN = a.SN, ExpiryDate = a.EXPIRYDATE.ToString("yyyy/MM/dd"), IMAGE = a.IMAGE, NAME = a.NAME });
            return all.ToList();
        }
        ...
        public List<object> GetAssCustomerAnalysisEChart(QueryAssCusandVenAnalysisInputDto inputDto)
        {
            var types = (from type1 in SMOWMSDbContext.AssetsTypes
                         where type1.TLEVEL == 1
                         select new
            ...
            if (t4 != null && t4.Count > 0)
            {
                ...
                resultList.Add(customerlist.Select(a => a.NAME).ToList());
                resultList.Add(t4);
                return resultList;
            }
            else
        ...
        public List<object> GetAssVendorAnalysisEChart(QueryAssCusandVenAnalysisInputDto inputDto)
        {
            var types = (from type1 in SMOWMSDbContext.AssetsTypes
                         where type1.TLEVEL == 1
                         select new
            ...
            {
                List<object> resultList = new List<object>();
                resultList.Add(vendorlist.Select(a => a.NAME).ToList());
                resultList.Add(t4);
                return resultList;
            }
            else return null;
        ...
        public List<object> GetAssQuantEChart(QueryAssQuantInputDto inputDto)
        {
            var asset = _assetsRepository.GetAll().Where(a => a.ISINWAREHOUSE == 1);
            if (!string.IsNullOrEmpty(inputDto.WAREID))
            {
            ...
            {
                resultList.Add(warelist.ToList());
                resultList.Add(asstypelist.ToList());
                resultList.Add(quantlist);
                return resultList;
            }
            else { return null; }
